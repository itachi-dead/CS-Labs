...
//Net 5 requaired (c# 9.0)
bool closeRequested = false;
bool playerTurn = true;
bool oneMoreGame = false;
...
while (!closeRequested)
{
	...
	//Checking for more games or quit
	oneMoreGame = false;
	if (!closeRequested)
	{
		Console.WriteLine();
		Console.WriteLine("Play Again [enter], or quit [escape]?");
	while(!closeRequested && !oneMoreGame)
...
void PlayerTurn()
{
	...
	while (!moved && !closeRequested)
	{
		...
		//setting the position of 'X' in game
		switch (Console.ReadKey(true).Key)
		{
